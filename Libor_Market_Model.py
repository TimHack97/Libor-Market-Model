...
# np.random.seed(40)                                                                # Set seed

# T         = Maturity time
# NoOfSteps = Number of steps in the time grid
# NoOfPaths = Number of forward rates
# Z         = Matrix containing standard normal random variables
def GenerateBM(T, NoOfSteps, NoOfPaths, insCorr):
    """"Generate Brownian motions."""
    dt = T / NoOfSteps                                                              # Timegrid
    
    Z = np.random.normal(0, 1, [NoOfPaths, NoOfSteps])                              # Create standard normal variables
    Zanti = -Z                                                                      # Antithetic variables
    
    W = np.zeros([NoOfPaths, NoOfSteps+1])                                          # Initialize Brownian motion
    Wanti = np.zeros([NoOfPaths, NoOfSteps+1])
...
# NoOfSteps = Number of steps in the time grid
# NoOfPaths = Number of forward rates
def insVol(NoOfSteps, NoOfPaths): 
    """Generate the instantaneous volatility matrix."""
    Vinit = np.random.uniform(0.2, 0.4, NoOfPaths)                                  # Draw from a uniform distribution every IV for every forward rate
    V = np.zeros([NoOfPaths, NoOfSteps]) 
       
    tenor = NoOfSteps / NoOfPaths                                                   # Tenor grid
    
    for i in range(NoOfSteps):                                                      # Loop per time step
        for j in range(NoOfPaths):                                                  # Loop per forward rate
            if i == 0:
...
# NoOfPaths = Number of forward rates
def insCorr(NoOfPaths):
    """Generate the instantaneous correlation matrix."""    
    N = NoOfPaths                                                                   # N is the total number of forward rates
...
def inFor(NoOfPaths): 
    ...
    L = np.random.uniform(0.02, 0.08, NoOfPaths)                                    # Draw from an uniform distribution between [0.2,0.8]
...
def GenerateFRWEul(NoOfSteps, NoOfPaths, T, tau, V, L0):
    ...
    C = insCorr(NoOfPaths)                                                          # Instantaneous correlation matrix
    BM, BManti = GenerateBM(T, NoOfSteps, NoOfPaths, C)                                  # Generated Brownian motions
    
    # Initialzie the forward rates
    FRW = np.zeros([NoOfPaths, NoOfSteps+1])                                        # NoOfSteps + 1 since we have time 0, 1, 2,.., N = N+1 steps
    FRW[:,0] = np.transpose(L0) 
    
    FRWanti = np.zeros([NoOfPaths, NoOfSteps+1])                                    # NoOfSteps + 1 since we have time 0, 1, 2,.., N = N+1 steps
    FRWanti[:,0] = np.transpose(L0) 


    for time in range(NoOfSteps):                                                   # Loop per time step
        for k in range(0,NoOfPaths):                                                # Loop per forward rate
            q = np.int(np.ceil((time*NoOfPaths) / NoOfSteps + 0.01))                # Smallest tenor point that is larger than the current time step
            X = 0
            Xanti = 0
            if q <= (k+1):  
...
# IC            = Instantaneous correlation matrix
# time          = Current time
def approxDrift(tau, FRW, bottom, frwrate, V, IC, time):
    ...
    for k in range(bottom, frwrate+1):
        ...
        Cij = getvalueC(V, IC, time, frwrate, sumrate, tau)                     # Calculate the C integral
        X += (((tau * FRW[k,time]) /    \
...
# V             = Instantaneous volatility matrix
# L0            = Initial forward rates
def GenerateFRWLS(NoOfSteps, NoOfPaths, T, tau, V, L0):
    """Generate forwward rates using a big time step."""
    # Obtain the initial values needed for the simulation
    IC = insCorr(NoOfPaths)                                                     # Instantaneous correlation matrix
    BM, BManti = GenerateBM(T, NoOfSteps, NoOfPaths, IC)                             # Generated Brownian motions
    
    # Initialize the forward rates
    FRW = np.zeros([NoOfPaths, NoOfSteps+1]) 
    FRW[:,0] = np.transpose(L0) 
    
    FRWanti = np.zeros([NoOfPaths, NoOfSteps+1]) 
    FRWanti[:,0] = np.transpose(L0)     
        
    for time in range(NoOfSteps):
        for frwrate in range(NoOfPaths): 
            q = np.int(np.ceil((time*NoOfPaths) / NoOfSteps + 0.01))                # Smallest tenor point that is larger than the current time step
            Cii = getvalueC(V, IC, time, frwrate, frwrate, tau)
            if q <= (frwrate+1):
                X1 = approxDrift(tau, FRW, (q-1), frwrate, V, IC, time)             # q-1 is the bottom of the summation sign in drift
                Y = -0.5 * Cii
                ...
                X2 = approxDrift(tau, FRW, (q-1), frwrate, V, IC, time+1)
                FRW[frwrate,time+1] = FRW[frwrate,time] * np.exp(((X1 + X2) / 2) + Y + Z)
                
                # Now using antithetic variables
                X1anti = approxDrift(tau, FRWanti, (q-1), frwrate, V, IC, time)             # q-1 is the bottom of the summation sign
                Yanti = -0.5 * Cii
                ...
                # Perform better approximation (Predictor-Corrector)
                X2anti = approxDrift(tau, FRWanti, (q-1), frwrate, V, IC, time+1)
...
# NoOfPaths = Number of forward rates that will be simulated
# T         = End date
# spotrate  = Spotrate
def capPriceMC(tau, resetT, K, V, V2, L, NoOfSteps, NoOfPaths, T, spotrate):
    ...
    for i in range(M):
        FRW = GenerateFRWEul(NoOfSteps*100, NoOfPaths, T, tau, V2, L)           # Obtain forward rates using Euler discretization
        FRWLS = GenerateFRWLS(NoOfSteps, NoOfPaths, T, tau, V, L)               # Obtain forward rates using a large time step
    
        # Calculate the payoffs using the forward rates from Euler discretization
        payoff = max(FRW[resetT-1, resetT] - K, 0)
        discount = (1 + tau * spotrate)
        for j in range(0,resetT):
            discount = discount * (1 + tau * FRW[j,j+1])
...
# T         = End date
# spotrate  = Spotrate
def discRate(tau, Tn, V, V2, L, NoOfSteps, NoOfPaths, T, spotrate):
    ...
    for i in range(M):
        FRW = GenerateFRWEul(NoOfSteps*100, NoOfPaths, T, tau, V2, L)           # Obtain forward rates using a Euler discretization
        FRWLS = GenerateFRWLS(NoOfSteps, NoOfPaths, T, tau, V, L)               # Obtain forward rates using a large time step
        
        # Obtain the rate using Euler discretization
        peul = (1 + tau * spotrate)
        for j in range(0,Tn-1):
            peul = peul * (1 + tau * FRW[j,j+1])
        eurate[i] = 1 / peul
...
def mainCalculation():
    ...
    NoOfPaths = int(T / tau)                                                        # Number of forward rates we want to generate
    
    # Note that normally you will obtain these using calibration, now I simulate them randomly
    V = insVol(NoOfSteps, NoOfPaths)
    V2 = insVol(NoOfSteps*1000, NoOfPaths)
    L = inFor(NoOfPaths)
    
    FRW = GenerateFRWEul(NoOfSteps*1000, NoOfPaths, T, tau, V2, L)                    # Generate forward rates using Euler discretization
    ...
    FRWLS = GenerateFRWLS(NoOfSteps, NoOfPaths, T, tau, V, L)                        # Generate forward rates using a large time-step
...
def pricingCaplet():
    ...
    # Note that normally you will obtain these using calibration, now I simulate them randomly
    V = insVol(NoOfSteps, NoOfPaths)
    V2 = insVol(NoOfSteps*100, NoOfPaths)
    L = inFor(NoOfPaths)
    spotrate = L[0] / 2
    
    [capEUL, capLS] = capPriceMC(tau, 3, 0.02, V, V2, L, NoOfSteps, NoOfPaths, T, spotrate)       # Obtain the simulated caplet prices
    ...
    print('The procentual difference is: ', 100 * (capLS - capBLACK) / capBLACK)

    print('Analytical price is: ', capBLACK, 'The Euler price is: ', capEUL)
    # print('The absolute difference is: ', np.abs(capBLACK - capEUL), '\n')
    print('The procentual difference is: ', 100 * (capEUL - capBLACK) / capBLACK, '\n')   
...
def generatingDiscountRates(): 
    ...
    L = inFor(NoOfPaths)
    spotrate = L[0] / 2       
    [anrate, eurate, lsrate] = discRate(tau, 3, V, V2, L, NoOfSteps, NoOfPaths, T, spotrate)      # Obtain the differenct calculatd zero-coupon rates
    
    print('Analytical rate is: ', anrate, 'The Large step rate is: ', lsrate)
    # print('The absolute difference is: ', np.abs(anrate - lsrate))
    print('The procentual difference is: ', 100 * (lsrate - anrate) / anrate)

    print('Analytical rate is: ', anrate, 'The Euler rate is: ', eurate)
    # print('The absolute difference is: ', np.abs(anrate - eurate), '\n')
    print('The procentual difference is: ', 100 * (eurate - anrate) / anrate, '\n')    
