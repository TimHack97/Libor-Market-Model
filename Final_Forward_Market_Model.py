    ...
    for time in range(NoOfSteps):
        for rate in range(NoOfRat): 
            ...
            if q <= (rate+1):                                                       # We say here rate+1 since in the literature the first rate is always 1 but python starts at 0
                ...
                Y = -0.5 * Cii
                Z = V[rate, 0] * (BM[rate , time+1] - BM[rate , time])              # Note: I just use the first volatility value since it is constant over time, also gamma = 1 here
...
def Generate_Backward_Rates(NoOfSteps, NoOfRates, T, tau, V, L0, eul_steps, vol):
        ...
        for rate in range(NoOfRates):                                                   # Loop per forward rate
            ...
            if rate > 0:
                ...
                for j in range(q-1, rate+1):                                                 # Perform the summation of the drift term
                    ...
                    Y = tau * IC[rate-1,j-1] * V[j-1,0] * BW_rate[j,steps+rate] * gamma
                    Z = 1 + tau * BW_rate[j,steps+rate]
                    X += Y / Z
    
                    Yanti = tau * IC[rate-1,j-1] * V[j-1,0] * BW_rate_anti[j,steps+rate] * gamma
                    Zanti = 1 + tau * BW_rate_anti[j,steps+rate]
                    Xanti += Yanti / Zanti  
                
                BW_rate[rate,steps+1+rate] = BW_rate[rate,steps+rate] + V[rate-1,0] * gamma_rate * BW_rate[rate,steps+rate] * X * dt + \
                    V[rate-1,0] * BW_rate[rate,steps+rate] * gamma_rate * (BM_new[rate,steps+1] - BM_new[rate,steps])                          # Perform the Euler discretization
                
                
                BW_rate_anti[rate,steps+1+rate] = BW_rate_anti[rate,steps+rate] + V[rate-1,0] * gamma_rate * BW_rate_anti[rate,steps+rate] * Xanti * dt + \
                    V[rate-1,0] * BW_rate_anti[rate,steps+rate] * gamma_rate * (BManti_new[rate,steps+1] - BManti_new[rate,steps])                          # Perform the Euler discretization
                ...
                Vol = vol
                'We dont need IC also above not and we need a new IV'
                ...
                for j in range(q-1, rate+1):                                                # Perform the summation of the drift term
                    ...
                    "I always use V[j,0], since the instantaneous volatility is constant I can just take the first value in column 0."
                    Y = tau * Vol * BW_rate[j,steps+rate] * gamma
                    Z = 1 + tau * BW_rate[j,steps+rate]
                    X += Y / Z
    
                    Yanti = tau * Vol * BW_rate_anti[j,steps+rate] * gamma
                    Zanti = 1 + tau * BW_rate_anti[j,steps+rate]
                    Xanti += Yanti / Zanti  
                
                BW_rate[rate,steps+1+rate] = BW_rate[rate,steps+rate] + Vol * gamma_rate * BW_rate[rate,steps+rate] * X * dt + \
                    Vol * BW_rate[rate,steps+rate] * gamma_rate * (BM_new[rate,steps+1] - BM_new[rate,steps])                                       # Perform the Euler discretization
                
                
                BW_rate_anti[rate,steps+1+rate] = BW_rate_anti[rate,steps+rate] + Vol * gamma_rate * BW_rate_anti[rate,steps+rate] * Xanti * dt + \
                    Vol * BW_rate_anti[rate,steps+rate] * gamma_rate * (BManti_new[rate,steps+1] - BManti_new[rate,steps])                          # Perform the Euler discretization
...
# vol       = Volatility of the first backward rate
def cap_price_Black(Notional, tau, T, strike, V, L, vol):  
...
# eul_steps = Number of Euler discretization steps
# vol       = Volatility of the first backward rate
def cap_price_MC(N, tau, strike, V, L, NoOfSteps, NoOfRates, T, M, eul_steps, vol):
    ...
    for i in range(M):
        [BW_rate, BW_rate_anti, xaxis] = Generate_Backward_Rates(NoOfSteps, NoOfRates, T, tau, V, L, eul_steps, vol)                 # Obtain backward rates using a large time step
        for resetT in range(0, NoOfRates):                                                   # Calculate every caplet price
            ...
            for k in range(0,resetT+1):
                discountLS     = discountLS * (1 + tau * BW_rate[k,k])
                discountLSanti = discountLSanti * (1 + tau * BW_rate_anti[k,k])
...
# eul_steps = Number of Euler discretization steps
# vol       = Volatility of the first backward rate
def zero_coupon_bond(tau, V, L, NoOfSteps, NoOfRat, T, M, eul_steps, vol):
    ...
    for i in range(M):
        [BW_rate, BW_rate_anti, x_axis] = Generate_Backward_Rates(NoOfSteps, NoOfRat, T, tau, V, L, eul_steps, vol)       # Obtain backward rates using simulation
        # print(BW_rate)
        for k in range(NoOfRat):                                                         # Loop to cover multiple ZCB       
            ...
            for j in range(0, k+1):
                pSim = pSim * (1 + tau * BW_rate[j, j])
                pSimanti = pSimanti * (1 + tau * BW_rate_anti[j, j])
...
def mainCalculation():
    ...
    BW_rate, BW_rate_anti, x_axis = Generate_Backward_Rates(NoOfSteps, NoOfRat, T, tau, V, L, eul_steps, vol)
    ...
    [capLS, se_LS] = cap_price_MC(Notional, tau, K, V, L, NoOfSteps, NoOfRat, T, M, eul_steps, vol)         # Obtain the simulated caplet prices
    capBlack = cap_price_Black(Notional, tau, T, K, V, L, vol)   
    ...
    [anrate, sim_rate, se_Sim] = zero_coupon_bond(tau, V, L, NoOfSteps, NoOfRat, T, M, eul_steps, vol)      # Obtain the differenct calculatd zero-coupon rates
