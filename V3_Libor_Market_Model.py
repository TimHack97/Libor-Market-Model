...
# spotrate  = Spotrate
def cap_price_Black(Notional, tau, T, K, V, L, spotrate):  
    ...
    for resetT in range(1, num_of_caps+1):                                                      # Apply Black's formula for every caplet            
        ...
        cap_price[i] = Notional * P * (L[resetT-1] * norm.cdf(d1) - K * norm.cdf(d2) )
...
def cap_price_MC(N, tau, K, V, V2, L, NoOfSteps, NoOfFor, T, spotrate, M, EulSteps):
...
def priceCaplet():
    ...
    EulSteps  = 1                                                                   # Euler discretization steps between two tenor points
...
def priceZCB(): 
    ...
    M         = 500                                                                 # Number of Monte Carlo simulations
    EulSteps  = 1                                                                 # Euler discretization steps
