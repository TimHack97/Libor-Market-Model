...
def Black(Notional, tau, K, L, vB, P):  
    ...
    cap_price = P * tau * Notional * (R * norm.cdf(d1) - K_hat * norm.cdf(d2) )
    return cap_price

# marketPrice   = Market price of a caplet
# tau           = Difference between time steps
# K             = Strike price
# L             = Rates
# sigmaInitial  = Initial guess for the implied value
# P             = discount factor
# Notional      = Notional
# resetT        = Reset time of the caplet
def ImpliedVolatility_back(marketPrice, tau, K, L, sigmaInitial, P, Notional, resetT):
    func = lambda sigma: np.power(Black(Notional, tau, K, L, sigma, P) - marketPrice, 1.0)             # Newton-Rhapson method
    vB   = optimize.newton(func, sigmaInitial, tol=1e-10, maxiter=200)                                 # Implied variance

    impliedVol = np.sqrt((vB) / (resetT * tau))                                                        # Implied volatility
...
def cap_price_Black_SABR(Notional, dif, strike, L, v, IV, resetT, rates, beta):  
    ...
    cap_price = P * dif * Notional * (R * norm.cdf(d1) - K * norm.cdf(d2) )
...
# pos        = [0.8, 0.85, 0.9, 0.95, 1]
pos        = [0.5, 0.55, 0.60, 0.65, 0.7]
...
for i in range(len(pos)):
    ...
    # plt.plot(strike, IV_back*100, label='{}'.format(pos[i]), marker=linestyles[i])
    plt.plot(strike, (vB), label='{}'.format(pos[i]), marker=linestyles[i])


plt.title('Implied volatility curve ($\mathregular{v^{B}}$)')
